      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            pageConfigItem = pagesConfig[pageId]; //console.log("THIS IS pageConfigItem =  " , JSON.stringify(pageConfigItem));
...
var pathMatchesPageId = function pathMatchesPageId(page, path, pageId, _ref3) {
  var pagesConfig = _ref3.pagesConfig;
  var pageRegexString = pagesConfig[pageId].regex; // Get the "regrex"'s value based on the pageId

  var pageRegex = new RegExp(pageRegexString); // get the "regex"'s value and turn it into a regular expressions string.
  // my code

  var evaluationResult = pageRegex.test(path); //  Test to see if the current path/route matches the "regex"'s value. 

  if (!evaluationResult) {
    //const { pathname: currentPath } = new URL(page.url())
    throw Error(" The current URL route DOES NOT MATCH the URL route specified in the pages.json.\n        Page Name                     = \"".concat(pageId, "\". \n        Current actual URL route      = \"").concat(path, "\".\n        Route specified in pages.json = \"").concat(pageRegex, "\".") //     `** Failed to get page name = ${pageId} from current route ${pageRegex}, 
...
var getCurrentPageId = function getCurrentPageId(page, globalConfig) {
  ...
  var pagesConfig = globalConfig.pagesConfig; // returns all page records that are specified in the "pages.json"

  console.log(" pagesConfig (in navigation-behaviors.ts) = ", pagesConfig); // My code

  var pageConfigPageIds = Object.keys(pagesConfig); // returns all page ID records from the "pages.json"
  ...
  var _URL2 = new URL(page.url()),
      currentPath = _URL2.pathname; // Note: this is where the problem is... Need to assign the name page ID to URL() object?
  ...
  if (!currentPageId) {
    throw Error("** Failed to get page name from current route ".concat(currentPath, ", \n          Possible pages: ").concat(JSON.stringify(pagesConfig)));
  }
